...
class Automatic_Georeferencing_System(QMainWindow):
    ...
    def on_mouse_move(self, event):
        ...
        self.compute()
        #self.Computation(XPoint,YPoint)
    def compute(self):  
        sift = cv2.SIFT_create()
        # Detect keypoints and compute descriptors for both images
        self.aerialkeypoints, self.aerialdescriptors = sift.detectAndCompute(self.aerialimg_data, None)
        self.tifkeypoint,self.tifdescriptor= sift.detectAndCompute(self.TIFFile.GetRasterBand(1).ReadAsArray(), None)
        bf = cv2.BFMatcher()
